...
{
    internal class BrittleCalculator
    {
        private static object[,] resultDataGrid = new object[17, 4];

        public static BeanBrittleResult calculateLevel1(BeanBrittle p_beanBrittle)
        {
        ...
        public static BeanBrittleResult calculateLevel2(BeanBrittle p_beanBrittle)
        {
            BeanBrittleResult beanBrittleResult = new BeanBrittleResult();
            beanBrittleResult.tg = p_beanBrittle.TheUncorrodedGoverningThickness;
            if (GlobalVar.UNIT_CURRENT == GlobalVar.UNIT_SI)
            ...
            bool[] resultLV = BrittleCalculator.getResultLV2(p_dblP_ratios, p_beanBrittle, beanBrittleResult);
            beanBrittleResult.resultDataGrid = BrittleCalculator.resultDataGrid;
            return beanBrittleResult;
        }

        private static bool[] getResultLV2(double[] p_dblP_ratios, BeanBrittle p_beanBrittle, BeanBrittleResult p_beanBrittleResult)
        {
            bool[] array = new bool[17];
            for (int i = 0; i < p_dblP_ratios.Length; i++)
            {
                double mATLV = BrittleCalculator.getMATLV2(i, p_dblP_ratios[i] * p_beanBrittle.designPressure.GetValueOrDefault(), p_beanBrittle, p_beanBrittleResult);
                ...
                {
                    array[i] = true;
                    BrittleCalculator.resultDataGrid[i, 3] = "Satisfied.";
                }
                else
                {
                    array[i] = false;
                    BrittleCalculator.resultDataGrid[i, 3] = "Not Satisfied.";
                }
            }
            return array;
        }

        private static double getMATLV2(int rowIndex, double p, BeanBrittle p_beanBrittle, BeanBrittleResult p_beanBrittleResult)
        {
            double num = 0.0;
            double num2 = Math.Pow(10.0, 6.0);
            double num3 = 6.895 * Math.Pow(10.0, 3.0);
            double num4 = 6.895 * Math.Pow(10.0, 6.0);
            BrittleCalculator.resultDataGrid[rowIndex, 0] = UnitUtils.getDisplayValue(new double?(p));
            double num5;
            ...
            if (p_beanBrittle.unitID == GlobalVar.UNIT_SI)
            {
                BrittleCalculator.resultDataGrid[rowIndex, 1] = UnitUtils.getDisplayValue(new double?((num17 - 32.0) * 0.55555555555555558));
            }
            else
            {
                BrittleCalculator.resultDataGrid[rowIndex, 1] = UnitUtils.getDisplayValue(new double?(num17));
            }
            if (num5 <= num15)
            {
                num = -155.0;
            ...
            if (p_beanBrittle.unitID == GlobalVar.UNIT_SI)
            {
                BrittleCalculator.resultDataGrid[rowIndex, 2] = UnitUtils.getDisplayValue(new double?((num - 32.0) * 0.55555555555555558));
            }
            else
            {
                BrittleCalculator.resultDataGrid[rowIndex, 2] = UnitUtils.getDisplayValue(new double?(num));
            }
            double result;
